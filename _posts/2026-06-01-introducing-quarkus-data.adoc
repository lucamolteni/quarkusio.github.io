...
Panache provided a repository pattern, a single place to define queries for each entity, and also offered an active record pattern to simplify data access by putting persistence methods directly to their entities.
...
This single extension gives you access to multiple data access styles, so you can pick the one that fits your application:

- **Active record** — persistence operations live directly on your entity. Define your queries in a nested interface, and the entity itself becomes the API. The simplest way to get started.
...
- **Raw SQL** — write native SQL queries using `@SQL` on a repository method, or inject a `StatelessSession` directly. No entity mapping required.
- **Managed session** — inject a `Session` for the full Hibernate experience: lazy loading, automatic dirty checking, batch inserts, and the first-level cache.
- **Reactive** — wrap your return types in `Uni` and Quarkus Data switches to Hibernate Reactive under the covers. The same entity model and repositories work in both blocking and non-blocking modes.

All of these styles share one key design principle: the entity type is always explicit in the return type.
When you write a method that returns `List<Book>`, the annotation processor knows the query targets `Book` — no need to repeat it in the query itself.
This is what enables compile-time validation across all styles, and what makes the code readable without annotations or configuration telling you which entity is involved.

While Quarkus Data Hibernate is a single extension, some features require an additional dependency to keep the default footprint small.
For example, reactive support requires adding `quarkus-hibernate-reactive` explicitly — if you don't need it, you don't pay for it.

// TODO link to the tutorial once it exists
We'll publish a hands-on tutorial shortly that walks through each style with working examples, from a first entity to a reactive pipeline.
...
If you're starting a new project, we'd encourage you to give it a try — it's the experience we think Quarkus users deserve.
Fair warning: the extension is still experimental, so you might run into a rough edge or two.
If you do, that's not a bug, that's a feedback opportunity (we tell ourselves).
